 ...
 *
 * section 5.4.3.2.2 Length-Limited Golomb-Power-of-2 Codeword of CCSDS123
 * standard encoder
 */
#include <assert.h>
#include <math.h>
 ...
 *
 * @param sample        sample to be encoded, note it's uint32_t size (will need
 * to be increased)
 ...
 */
uint32_t encode_sample_optimized(uint32_t sample, unsigned int k,
  ...
  // 		- go by "first 1", that's the start of the sample, stored in 32 bit
  // integer
  /**
   * number of bits used
  ...
  } else { // if don't need unary portion, then just use k bits, always plus
           // stop character
    *num_bits_used = k + 1;
  }
...
uint32_t decode_sample(uint32_t code, unsigned int k) {
  uint32_t ret;
  int i =
      32; // indexing variable, start at bit 32 (left edge) and work backwards
  uint32_t quotient = 0;
...
void encode_mymatrix(mymatrix *mat, int k, char *filename) {
  ...
  for (i = 0; i < nrows; i++) {
    for (j = 0; j < ncols; j++) {
       ...
       * note that the bitfile library BitFilePutBits() (plural) gives some
       * weird padding issue
       *     - breaks assumptions about binary file structure
       * falling back to use the singular BitFilePutBit() and looping through
       * --> avoids padding issue
       */
 ...
 * @param k             must know divisor size for decoding, M = 2^k, k control
 * parameter
...
uint32_t decode_sample_bitfile(bit_file_t *stream, unsigned int k) {
  ...
  if (c == 0) {
    ...
    for (i = 0; i < k; i++) {
      truncated_binary =
          truncated_binary | (c << (k - i)); // add in backwards order, since
                                             // reading bits in backwards order
      c = BitFileGetBit(stream);             // get next bit
    }
    truncated_binary =
        truncated_binary | (c << (k - i)); // need to do it for the last bit
  } else {
  ...
  for (i = 0; i < nrows; i++) {
    for (j = 0; j < ncols; j++) {
      ...
      if (t == 0) {
        ...
        k = k -
            1; // went over the limit to exit the while loop, need to subtract 1
      ...
      // ====== debugging =========
      // logger("DEBUG", "(%d, %d): sample=%3d | code=%-8x |
      // num_bits_used=%3d\n", i, j, sample, code, num_bits_used);
      // print_binary_32(code);
      // logger("DEBUG", "counter=%3d \t accum=%3d \t k=%3d \t left_side=%3d \t
...
void check_single_decode(void) {
   ...
   * divisor [M]: 16
   * ===== encoding =====
   * sample: decimal = 18,   bin = 00000000 00000000 00000000 00010010,      hex
   * = 12 encoded:                bin = 00100010,         hex = 22
   */
  uint32_t code = 0x344;
  unsigned int k = 7;
 ...
 * @param k                 divisor control parameter M = 2^k
 * @param decoded_array     decoded array put into unsigned 32-bit array for
 * easy access
 * @param decoded_size      input, number of samples is known a priori (from
 * header file)
 */
void check_multiple_decode(char *filename, int k, uint32_t *decoded_array,
                           uint32_t decoded_size) {
  uint32_t counter = 0; //  number of currently decoded samples, amount of
                        //  samples is known a priori (aka in file header)
  uint32_t decoded;
