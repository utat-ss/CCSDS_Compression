...
void logger_init(const char *filename) {
  ...
  // char sbuf[1000];
  // char WORK_ROOT_DIR[] =
  // "/home/liyongda/Documents/GitHub/utat/CCSDS_Compression/integration/";
...
void logger(const char *tag, const char *format, ...) {
  ...
  // some black magic to deal with printf arguments --> aka the (...) in the
  // function arguments
  ...
  if (LOG_TO_FILE) {
    ...
    va_start(
        args,
        format); // need to call va_start 2x if logging both to terminal and
                 // file, I think it's cuz the print thing is destructive
...
void print_binary_32(uint32_t word) {
  logger("DEBUG",
         "bin = " BYTE_TO_BINARY_PATTERN " " BYTE_TO_BINARY_PATTERN
         " " BYTE_TO_BINARY_PATTERN " " BYTE_TO_BINARY_PATTERN ",\n",
         BYTE_TO_BINARY(word >> 24), BYTE_TO_BINARY(word >> 16),
         BYTE_TO_BINARY(word >> 8), BYTE_TO_BINARY(word));
}
