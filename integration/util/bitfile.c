 ...
 */
typedef int (*num_func_t)(bit_file_t *, void *, const unsigned int,
                          const size_t);
...
int BitFilePutBit(const int c, bit_file_t *stream) {
  ...
  if (c != 0) {
    stream->bitBuffer |= 1; // default
    // stream->bitBuffer = (stream->bitBuffer) | 0x1<<8; // Yong Da messing
    // around
  }
...
int BitFilePutBits(bit_file_t *stream, void *bits, const unsigned int count) {
  ...
  if (remaining != 0) {
    ...
    while (remaining > 0) {
      ...
      // returnValue = BitFilePutBit((tmp & 0x08), stream);   // Yong Da messing
      // around
