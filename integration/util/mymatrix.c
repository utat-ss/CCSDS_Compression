...
float mat_get(mymatrix *mat, int i, int j) {
  if (mat == NULL) {
    ...
    logger("ERROR",
           "out of range, matrix size is (%d, %d), trying to access row=%d\n",
           mat->nrows, mat->ncols, i);
    return -1;
  } else if (j > mat->ncols || j < 0) {
    logger("ERROR",
           "out of range, matrix size is (%d, %d), trying to access col=%d\n",
...
float mat_get_flat(mymatrix *mat, int i) {
  if (mat == NULL) {
    ...
    logger("ERROR",
           "out of range, matrix size is (%d, %d) = %d, trying to access "
           "element=%d\n",
...
float vec_get(myvector *vec, int i) {
  if (vec == NULL) {
    ...
    logger("ERROR",
           "out of range, vector size is (%d), trying to access row=%d\n",
           vec->size, i);
    return -1;
  }
...
void vec_set(myvector *vec, int i, float val) { vec->data[i] = val; }
...
float vec_dot_prod(myvector *vecA, myvector *vecB) {
  ...
  if (vecA->size != vecB->size) {
    logger("ERROR", "dot product size mismatch, (%d) vs (%d)\n", vecA->size,
           vecB->size);
    // printf("dot product size mismatch, (%d) vs (%d)", vecA->size,
    // vecB->size);
  }
