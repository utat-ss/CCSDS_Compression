...
myvector *compute_local_diff_vector(datacube *cube, int z, int x, int y) {
  ...
  float ls = local_sum(mat, x, y); // can't name the variable `local_sum` since
                                   // it conflicts with function definition??

  // local difference values, edge case is they're 0 if on the top row of the
  // frame (y==0)
  float d_n = 0;
  float d_w = 0;
  float d_nw = 0;
  ...
  if (x == 0 & y > 0) {
    ...
    d_w = 4 * mat_get(mat, x, y - 1) -
          ls; // on left side, can't go West, so go North
    d_nw = 4 * mat_get(mat, x, y - 1) -
           ls; // on left side, can't go West, so go North
  }
  // on top row of frame
  else if (y == 0) {
  ...
  // fill the rest of the vector with central differences (differences between
  // spectral bands)
  int z_idx = 0;
...
float predict_calc(datacube *datacube, int z, int x, int y) {
  ...
  float arg0 =
      d + pow(2, PARAM_OMEGA) *
              (local_sum(cube_get_frame(datacube, z), x, y) - 4 * s_mid);
  float arg1 = mod_star(arg0, PARAM_R);
  float arg2 = pow(2, PARAM_OMEGA + 2) * s_mid + pow(2, PARAM_OMEGA + 1);

  float ret = clip(arg1 + arg2, pow(2, PARAM_OMEGA + 2) * s_mid,
                   pow(2, PARAM_OMEGA + 2) * s_max + pow(2, PARAM_OMEGA + 1));
