...
// technically don't need the "util/" prefixes, since I handle it in the GCC
// include paths -I argument but including it for clarity
...
void check_encoder(int argc, char *argv[]) {
  ...
  if (argc != 3) {
    printf(
        "usage is [number of rows], [number of columns], expect 2 arguments\n");
    return;
  }
  ...
  // uint32_t* decoded_array = (uint32_t*) malloc(sizeof(uint32_t) *
  // nrows*ncols); check_multiple_decode("output/encoded.bin", k, decoded_array,
  // nrows*ncols);
...
void check_local_diff_vector() {
  ...
  // the (0,0,0) value is always kept as the raw value, since it's used for
  // reference to decode everything else
...
void check_data_parsing(void) {
  char filename[] = "/Users/liyongda/Documents/GitHub/CCSDS_Compression/"
                    "integration/images/indian_pines.txt";
  // char filename[] =
  // "/Users/liyongda/Documents/GitHub/CCSDS_Compression/integration/output/random_datacube_formatted.txt";

  datacube *cube = parse_cube_from_file(filename);
  // pretty_print_cube(cube);
  save_cube(cube, "/Users/liyongda/Documents/GitHub/CCSDS_Compression/"
                  "integration/output/parse_check_indian_pines.txt");
}
