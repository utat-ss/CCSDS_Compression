    ...
    for z in range(0, Nz):
        for y in range(0, Ny):
            for x in range(0, Nx):
                ...
                weight_vector_new = np.empty(0)
                weight_vector_new = comp.weight_update(dr_samp, predicted_sample, predicted_residual, t, Nx, w_prev,
                                                       weight_vector_new, ld_vector, z, Nz)
    show_data(data, "data")
...
def decoder_visualize(data):
    ...
    print(mapped)
    encoded = comp.encoder(mapped)
    print(np.array(data).size/np.array(encoded).size)
...
def unpredict_visualize(data):
    ...
    return encoded, data_final


def get_metrics(file_name, data_header, subset_size=100):
    """
    Calculates compression ratio and RMSE for compression of given data
    :param file_name: file location for hyperspectral dataset
    :param data_header: header name used in data files
    :param subset_size: size of subset to be used for compression

    """
    hs_data = load_pic(file_name, data_header)
    ...
    compression_ratio = len(np.asarray(hs_data).tobytes() * 8) / len(encoded)
